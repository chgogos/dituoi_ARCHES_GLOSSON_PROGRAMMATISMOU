...
* Αυξημένη δυνατότητα έκφρασης ιδεών. Για παράδειγμα προγραμματιστής της C που γνωρίζει τη χρήση των λεξικών στην Python μπορεί να σχεδιάσει παρόμοιες δομές ή να αναζητήσει βιβλιοθήκες που υλοποιούν παρόμοιες δομές στη C όπως η [GLib - Hash Tables](https://developer.gnome.org/glib/stable/glib-Hash-Tables.html).
...
```{.c title="orthogonality_violation1.c" linenums="1"}
--8<-- "src/c/orthogonality_violation1.c"
```
...
```{.c title="orthogonality_violation2.c" linenums="1"}
--8<-- "src/c/orthogonality_violation2.c"
```
...
```{.c title="orthogonality_violation3.c" linenums="1"}
--8<-- "src/c/orthogonality_violation3.c"
```

Μεταγλώττιση και εκτέλεση του κώδικα:

```{.text .nocopy}
$ gcc orthogonality_violation3.c
$ ./a.out
42
fun()
fun()
...
```{.c title="orthogonality_violation4.c" linenums="1"}
--8<-- "src/c/orthogonality_violation4.c"
```

Μεταγλώττιση και εκτέλεση του κώδικα:

```{.text .nocopy}
$ gcc orthogonality_violation4.c
$ ./a.out
0
1 0 0
```
...
```{.c title="orthogonality_violation5.c" linenums="1"}
--8<-- "src/c/orthogonality_violation5.c"
```

Μεταγλώττιση και εκτέλεση του κώδικα:

```{.text .nocopy}
$ gcc orthogonality_violation5.c
$ ./a.out
4
...
```{.cpp title="alias.cpp" linenums="1"}
--8<-- "src/cpp/alias.cpp"
```
...
* Η αρχιτεκτονική von Neumann (δεδομένα και προγράμματα στην ίδια μνήμη, ξεχωριστή Κεντρική Μονάδα Επεξεργασίας από τη μνήμη) επηρέασε τη σχεδίαση των γλωσσών προγραμματισμού και οδήγησε στις προστακτικές γλώσσες (Fortran, C, C++, Java, Python).
    * μεταβλητές &rarr; κελιά μνήμης
    * εκχώρηση &rarr; προσκόμιση δεδομένων από τη μνήμη στον επεξεργαστή, επεξεργασία, αποθήκευση αποτελεσμάτων στη μνήμη
    * επανάληψη &rarr; μέσω εντολών διακλάδωσης
...
* Τα προγράμματα μεταφράζονται σε γλώσσα μηχανής και στη συνέχεια εκτελούνται ταχύτατα.
    1. πηγαίος κώδικας &rarr; λεκτικός αναλυτής &rarr; λεκτικές μονάδες 
    2. λεκτικές μονάδες &rarr; συντακτικός αναλυτής &rarr; δένδρο συντακτικής ανάλυσης
    3. δένδρο συντακτικής ανάλυσης &rarr; γεννήτρια ενδιάμεσου κώδικα (σημασιολογικός αναλυτής + βελτιστοποιητής) &rarr; ενδιάμεσος κώδικας
    4. ενδιάμεσος κώδικας &rarr; γεννήτρια κώδικα &rarr; γλώσσα μηχανής
...
```{.c title="preprocessor1.c" linenums="1"}
--8<-- "src/c/preprocessor1.c"
```
