...
Παράδειγμα στη C++ με προκαθορισμένες τιμές παραμέτρων (οι παράμετροι με προκαθορισμένες τιμές πρέπει να εμφανίζονται στο τέλος στη λίστα παραμέτρων της συνάρτησης)
...
Παράδειγμα με τοπικές μεταβλητές (στατική μεταβλητή και δυναμική-στοίβας) στη C++
...
Παράδειγμα συνάρτησης που δέχεται έναν δισδιάστατο πίνακα και επιστρέφει τον ανάστροφό του (οι γραμμές γίνονται στήλες και ο στήλες γραμμές).

```{.py title="transpose_matrix.py" linenums="1"}
--8<-- "src/python/transpose_matrix.py"
```

```console
$ python traspose_matrix.py
Original matrix:
[1, 2, 3]
[4, 5, 6]
...
Αριθμητικός τρόπος υπολογισμού ολοκληρωμάτων στο διάστημα $[0, 10]$ για τις συναρτήσεις:
...
Σε κάθε περίπτωση η κατάλληλη συνάρτηση περνά ως τελευτάια παράμετρος στη συνάρτηση υπολογισμού `trapezio`.

```{.c title="trapezoid_rule.c" linenums="1"}
--8<-- "src/c/trapezoid_rule.c"
```

Ισοδύναμος με τον παραπάνω κώδικα σε Python:

```{.py title="trapezoid_rule.py" linenums="1"}
--8<-- "src/python/trapezoid_rule.py"
```

```
$ python trapezoid_rule.py
result: 50.00000
result: 333.33333
result: 2500.00000
result: 22025.46579
...
```{.cpp title="generic_sort.cpp" linenums="1"}
...
Παράδειγμα υπερφόρτωσης τελεστών στην Python. Υλοποίηση πράξεων +, -, * για  διαστήματα τιμών (intervals). Δείτε την περιγραφή των πράξεων με διαστήματα στο <https://en.wikipedia.org/wiki/Interval_arithmetic>.


```{.py title="interval.py" linenums="1"}
--8<-- "src/python/interval.py"
```
...
```{.py title="closure5a.py" linenums="1"}
--8<-- "src/python/closure5a.py"
```

```{.py title="closure5b.py" linenums="1"}
--8<-- "src/python/closure5b.py"
```

```{.py title="closure5c.py" linenums="1"}
--8<-- "src/python/closure5c.py"
```

## 9.13 Συρρουτίνες (coroutines)
...
Οι coroutines είναι ένας τύπος συνάρτησης που επιτρέπει την παύση και τη συνέχεια της εκτέλεσης. Στην Python οι coroutines υλοποιούναι με τις λέξεις κλειδιά async, await από τη βιβλιοθήκη asyncio και επιτρέπουν τη συγγραφή ασύγχρονου κώδικα.

Παράδειγμα με 2 tasks που εκτελούνται ταυτόχρονα (concurrently). Το keyword await προκαλεί παύση εκτέλεσης της τρέχουσας coroutine μέχρι το αντικείμενο που δέχεται ως όρισμα να ολοκληρώσει την εργασία του.

```{.py title="coroutines.py" linenums="1"}
--8<-- "src/python/coroutines.py"
```
